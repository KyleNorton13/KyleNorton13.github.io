function init() {
  ...
  d3.json("Challenge/js/data/samples.json").then((data) => {
...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var yticks = ids.map(objectSample => "OTU" + objectSample).slice(0,10).reverse();
...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var bubbleData = [{x:ids,y:sample_values, text:labels,mode:'markers',
      marker:{size:sample_values,color:ids, colorscale:'Earth'}
    }
